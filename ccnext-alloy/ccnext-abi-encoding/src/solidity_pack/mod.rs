use super::common::{compute_v, compute_y_parity, encode_blob_hashes, AbiEncodeResult};
use alloy::{
    consensus::{
    ...
    let result = vec![
        ...
        DynSolValue::Array(
            rx.inner
                ...
                .iter()
                .map(|log| {
                    // this closure, turns the log into a bytes
...
pub fn solidity_packed_encode(
    ...
    let field_type: Vec<String> = all_fields
        .into_iter()
